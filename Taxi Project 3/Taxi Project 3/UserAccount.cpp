...
void userAccount(string userName)
{
	...
	while (runAccount == true) {
		...
		cout << "\nInput 1 to Book a Taxi|Input 2 to Review TOS|Input 3 to View Your rights|Input 4 to Make a Review|Input 5 to Report a Problem\nInput 6 to Re-read Pricing Calculation|Input 7 to Re-read Available Areas|Input 8 to Access Lost & Found|Input 9 to Cancel a Booking|Input 10 to go Back to menu\n:";
		cin >> menuOption;
		try
		{
			switch (menuOption)
			...
			case 9:
				...
				logCancellation("cancellation_report", userName, target);
				readCSV("bookings");
				break;
			case 10:
				runAccount = false;
